...
$getdsva=get-vm -name *DSVA* | select name, vmhost, MemoryGB, powerstate | ConvertTo-HTML -Fragment
#--------------------------------------------------------------------------------------------------------------------------------------
#List Status of Temp EdgeGateway VM's.
$Reportremp="<div id='boxheader'>Check Temp Edge Gateways </div>"
$gettemp=get-vm -name temp* | select name, vmhost, MemoryGB, powerstate | ConvertTo-HTML -Fragment
#--------------------------------------------------------------------------------------------------------------------------------------
# Check Sites URL
$Reporturl ="<div id='boxheader'>Url Check for $Location</div>"
...
foreach ($esxihost in $hosts){ 												#For each Host
    foreach($triggered in $esxihost.TriggeredAlarmState){ 					#For each triggered alarm of each host
            ...
            write-host $arrayline.AlarmType
